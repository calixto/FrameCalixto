...
class controlePadrao extends controle {
     ...
     * @param string $grupo
     */
    public static function registrarInternacionalizacao($entidade, $visualizacao, $grupo = '') {
        $inter = definicaoEntidade::internacionalizacao($entidade);
        $entidade = definicaoEntidade::entidade($entidade);
        $inter = new $inter();

        $visualizacao->{"{$grupo}titulo"} = $inter->pegarTituloSistema();
        $visualizacao->{"{$grupo}subtitulo"} = $inter->pegarSubtituloSistema();
        if ($inter->pegarTitulo()) {
            $visualizacao->{"{$grupo}tituloEspecifico"} = sprintf(
                    '%s - %s', $inter->pegarTitulo(), $inter->pegarTexto(definicaoEntidade::funcionalidade(controle::controleAcessado()))
            );
        } else {
            $visualizacao->{"{$grupo}tituloEspecifico"} = $inter->pegarTexto(definicaoEntidade::funcionalidade(controle::controleAcessado()));
        }
    ...
    public static function obterValorDoComponenteHtmlPadrao($campo, $valor) {
        switch (strtolower($campo['componente'])) {
            ...
            case 'data e hora':
                switch (true) {
                    case($valor['data'] && $valor['hora']) : $valor = new TDataHora(implode(' ', $valor));
                        break;
                    case($valor['data']) : $valor = new TDataHora($valor['data']);
                        break;
                    case($valor['hora']) : $valor = new TDataHora(TData::hoje($valor['hora']));
                        break;
                    default:
                        $valor = null;
                }
